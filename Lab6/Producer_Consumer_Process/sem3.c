...
int main()
{
    ...
    *tail=0;
    ...
    if(fork()==0)
    {
        ...
        //link the shared mem  for producer
        ...
        while(1)
        {
		    ...
		    *tail=(*tail+1)%n;
		    ...
		    //remove sleep to see better result as there is only one producer
    ...
    else
    {
        ...
        if(fork()==0)
        {
            ...
            fork();
            fork();
            //link the shared mem  for multiple consumers
            queue=(int *)shmat(shmID,NULL,0);
            if (queue == (int *)-1 )
            {
            ...
            while(1)
            {
		        ...
		        item=queue[*head];
		        *head=(*head+1) % n;
		        printf("\t\t\t\tconsumed product number: %d by %d\n",item,getpid());
		        sem_post(sem_lock);
