...
class comparator_burst_left
{
	public:
	bool operator () (Process_class* p1,Process_class* p2)
	{
		if(p1->remaing_time > p2->remaing_time)
			return true;
		return false;
	}
};
...
priority_queue<Process_class*, vector<Process_class*>, comparator_burst_left > process_queue;
...
void helperFunction()
{
	...
	while(true)
	{
		...
		if(  !process_queue.empty() && process_queue.top()->remaing_time==0)
		{
			...
			process_queue.pop();
			//sort(process_queue.begin(),process_queue.end(),comparator_burst_left);
		}
