...
queue <Process_class* > process_queue;
...
//		cout<<" "<<process_queue.front()->pid<<" \n";
		
		while(first_time_process<total_process && process[first_time_process].arrival_time==system_time)
		{
			process[first_time_process].time_quantum=TIMESLICE;
			cout<<" "<<process[first_time_process].pid<<" \n";
			process_queue.push(&process[first_time_process]);
			//process_queue.front()->time_quantum=2;
		...
		if(!process_queue.empty()  && process_queue.front()->time_quantum==0)
		{
				if(process_queue.front()->remaing_time!=0)
				{
					process_queue.front()->time_quantum=TIMESLICE;
		...
		if(  !process_queue.empty() && process_queue.front()->remaing_time==0)
		{
			...
			{	
				cout<<"terminated:"<<process_queue.front()->pid<<" \n";
		...
		if(  !input_queue.empty() && input_queue.front()->remaing_time==0)
		{
			...
			{
				cout<<"terminated:"<<input_queue.front()->pid<<" \n";
			...
			{
				input_queue.front()->time_quantum=TIMESLICE;
		...
		if(  !output_queue.empty() && output_queue.front()->remaing_time==0)
		{
			...
			{	
				...
				cout<<"terminated:"<<output_queue.front()->pid<<" \n";
			...
			{
				output_queue.front()->time_quantum=TIMESLICE;
		...
		{	
			...
			if(process_queue.front()->time_quantum!=0)
			{
				process_queue.front()->time_quantum=process_queue.front()->time_quantum-1;
			}


			process_queue.front()->remaing_time=process_queue.front()->remaing_time-1;
			printf("p queue   %d %d %d \n",process_queue.front()->pid,process_queue.front()->remaing_time,process_queue.front()->time_quantum);
		}
...
{	
	...
	//cout<<"ashish\t";
	//process[0].time_quantum=TIMESLICE;
