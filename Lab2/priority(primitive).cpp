...
{
	public:
	bool operator () (Process_class* p1,Process_class* p2)
	{
...
priority_queue<Process_class*, vector<Process_class*>, comparator_priority > process_queue;
...
void helperFunction()
{
	...
	while(true)
	{
		...
		if(  !process_queue.empty() && process_queue.top()->remaing_time==0)
		{
		
			if(process_queue.top()->counter_burst==process_queue.top()->burst_time.size()-1)
			{	
			
				process_queue.top()->turn_around_t=system_time - process_queue.top()->arrival_time;
			}
			else if(process_queue.top()->burst_time[process_queue.top()->counter_burst+1].first=='O' )
			{
				output_queue.push(process_queue.top());
				process_queue.top()->remaing_time=process_queue.top()->burst_time[process_queue.top()->counter_burst+1].second;
				process_queue.top()->counter_burst++;
			}
			else if (process_queue.top()->burst_time[process_queue.top()->counter_burst+1].first=='I' )
			{
				input_queue.push(process_queue.top());
				process_queue.top()->remaing_time=process_queue.top()->burst_time[process_queue.top()->counter_burst+1].second;
				process_queue.top()->counter_burst++;
			}
		...
		if(  !input_queue.empty() && input_queue.front()->remaing_time==0)
		{
			...
			{
				process_queue.push(input_queue.front());
				input_queue.front()->remaing_time=input_queue.front()->burst_time[input_queue.front()->counter_burst+1].second;
				input_queue.front()->counter_burst++;
			}
		...
		if(  !output_queue.empty() && output_queue.front()->remaing_time==0)
		{
			...
			{
				process_queue.push(output_queue.front());
				output_queue.front()->remaing_time=output_queue.front()->burst_time[output_queue.front()->counter_burst+1].second;
				output_queue.front()->counter_burst++;
			}
		...
		while(first_time_process<total_process && process[first_time_process].arrival_time==system_time)
		{
			...
			process_queue.push(&process[first_time_process]);
		...
		{	
			...
			if(process_queue.top()->first_response_start==0)
			{
				process_queue.top()->response_t=system_time - process_queue.top()->arrival_time;
				process_queue.top()->first_response_start=1;
			}
			process_queue.top()->remaing_time=process_queue.top()->remaing_time-1;
			printf("p queue   %d %d\n",process_queue.top()->pid,process_queue.top()->remaing_time);
		}
...
{	
	...
	cout<<"pid\tprio\tarr_t\tresp_t\twait_t\tturn_at\n";
	for(int i=0;i<total_process;i++)
		cout<<process[i].pid<<"\t"<<process[i].priority<<"\t"<<process[i].arrival_time<<"\t"<<process[i].response_t<<"\t"<<process[i].wait_t<<"\t"<<process[i].turn_around_t<<"\n";
